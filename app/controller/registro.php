...
if ($_POST) {
    if (isset($_POST['nombre']) && !empty($_POST['nombre']) && 
        ...
        if(is_numeric($_POST['nombre'])) {
            ...
            $insercion = $conexion->prepare("INSERT INTO t_usuarios (nombre,apellido,email,pass)  VALUES(:nombre,:apellido,:email,:pass)");
            ...
            if ($insercion) {
                echo json_encode([1," registrado correctamente"]);
            } else {
                echo json_encode([0," NO registrado"]);
            }
        }
        
    } else {
        echo json_encode([0,"oye te falta algo 🤨"]);
    }
}
